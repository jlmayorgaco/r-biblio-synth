...
m3c_ieee_theme <- function(base_size = 8.5, base_family = NULL) {
...
# ──────────────────────────────────────────────────────────────────────────────
# 2) Share trend: two stacked 100% bars (TP/TC) with IEEE swatch legend
#    GUARANTEES each period sums to 100% by using position="fill" on counts.
# ──────────────────────────────────────────────────────────────────────────────
# Keep this helper in the same file (used below)
.m3c_continent_df_guard <- function(df_cy) {
  need <- c("continent","year","TP","TC")
  if (!all(need %in% names(df_cy))) {
    stop("df_cy must contain columns: continent, year, TP, TC.")
...
# ──────────────────────────────────────────────────────────────────────────────
# 2-period continent share + absolute plots (TP & TC) — IEEE style
# ──────────────────────────────────────────────────────────────────────────────
m3c_plot_continent_share_trend <- function(df_cy,
                                           ...
                                           label_size  = 2.6,
                                           label_fmt   = function(x) scales::percent(x, accuracy = 1),
                                           csv_digits  = 4) {
  # Guard & inputs
  df_cy <- .m3c_continent_df_guard(df_cy)
  ...
  # Legend order = latest-period ranking
  ord_from_latest <- function(dat) {
    lv <- dat %>% dplyr::filter(Period == p2$label) %>%
      dplyr::arrange(dplyr::desc(Value)) %>%
      dplyr::pull(continent) %>% unique()
    unique(c(lv, setdiff(all_cont, lv)))
  }
  ord_tp <- ord_from_latest(tp_share)
  ord_tc <- ord_from_latest(tc_share)

  # Helpers to build plots -----------------------------------------------------
  # Compact, IEEE-ish title + labels at the TOP of each stacked segment
build_share_plot <- function(dat, ord, ttl_suffix,
                             label_size = 2.6,
                             label_pad = 0.01,
                             label_threshold = 0.05,
                             small_alpha = 0.20) {

  dat$continent <- factor(dat$continent, levels = ord)
  ...
  ggplot2::ggplot(
    dat,
    ggplot2::aes(x = Period, y = Value, fill = continent, alpha = .big)
  ) +
    ggplot2::geom_col(position = "stack", colour = "black", linewidth = 0.25, width = 0.75) +
    ggplot2::geom_text(
    ...
    ggplot2::scale_y_continuous(
      ...
      limits = c(0, 1 + label_pad), breaks = c(0,.25,.5,.75,1), expand = c(0, 0.02)
    ) +
  ...
  label_num <- scales::label_number(big.mark = ",", accuracy = 1)
  build_abs_plot <- function(dat, ord, ttl_suffix) {
    label_num <- scales::label_number(big.mark = ",", accuracy = 1)
    ggplot2::ggplot(dat, ggplot2::aes(x = Period, y = Value, fill = factor(continent, levels = ord))) +
      ggplot2::geom_col(position = "stack", width = 0.75, colour = "black", linewidth = 0.25) +
      ggplot2::scale_y_continuous(labels = label_num, expand = c(0.02, 0.02)) +
      ggplot2::scale_fill_grey(start = 0.9, end = 0.2, name = NULL) +
      ggplot2::labs(
        x = NULL, y = NULL,
        title = if (is.null(title))
          sprintf("Continent Totals of %s (%s–%s vs %s–%s)", ttl_suffix, p1$start, p1$end, p2$start, p2$end)
        else sprintf("%s — %s (Totals)", title, ttl_suffix)
      ) +
      m3c_ieee_theme() +
      ggplot2::theme(legend.position = "none")  # <-- no legend in the panel
  }

ord_global <- tp_shares |>
  ...
  # Build plots
p_tp_share <- build_share_plot(tp_shares, ord_global, "Publications (TP)",
                               label_size = 2.8, label_pad = 0.015,
                               label_threshold = 0.1, small_alpha = 0.10)

p_tc_share <- build_share_plot(tc_shares, ord_global, "Citations (TC)",
                               label_size = 2.8, label_pad = 0.015,
                               label_threshold = 0.1, small_alpha = 0.10)
  p_tp_abs   <- build_abs_plot(tp_abs, ord_global, "Publications (TP)")
  p_tc_abs   <- build_abs_plot(tc_abs, ord_global, "Citations (TC)")
...
m3c_continent_trends_grid_2x2 <- function(df_cy,
  ...
  res <- m3c_plot_continent_share_trend(
    df_cy, periods = periods, out_base = NULL, kind = kind, aspect = aspect
  )

  ieee_family <- "Times"
  p_tp_share <- res$plots$tp_share + ggplot2::theme(text = ggplot2::element_text(family = ieee_family))
  p_tc_share <- res$plots$tc_share + ggplot2::theme(text = ggplot2::element_text(family = ieee_family))
  p_tp_abs   <- res$plots$tp_absolute +
    ggplot2::labs(title = sprintf("Continent Totals of Publications (TP)\n(%s vs %s)",
                                  periods[[1]]$label, periods[[2]]$label)) +
    ggplot2::theme(text = ggplot2::element_text(family = ieee_family),
                   plot.title = ggplot2::element_text(size = 9, face = "bold", hjust = 0.5))
  p_tc_abs   <- res$plots$tc_absolute +
    ggplot2::labs(title = sprintf("Continent Totals of Citations (TC)\n(%s vs %s)",
                                  periods[[1]]$label, periods[[2]]$label)) +
    ggplot2::theme(text = ggplot2::element_text(family = ieee_family),
                   plot.title = ggplot2::element_text(size = 9, face = "bold", hjust = 0.5))

  # Re-enable legend ONLY on a copy for extraction (bottom / horizontal)
  p_for_legend <- p_tp_share +
    ggplot2::theme(
      legend.position   = "bottom",
      legend.direction  = "horizontal",
      legend.margin     = ggplot2::margin(t = 1, r = 0, b = 1, l = 0),
      legend.box.margin = ggplot2::margin(t = 0, r = 0, b = 0, l = 0),
      legend.key.height = grid::unit(6, "pt"),
      legend.key.width  = grid::unit(10, "pt"),
      legend.text       = ggplot2::element_text(size = 7, family = ieee_family)
    ) +
    ggplot2::guides(fill = ggplot2::guide_legend(
      title = NULL, direction = "horizontal", nrow = 1, byrow = TRUE,
      keyheight = grid::unit(6, "pt"), keywidth = grid::unit(10, "pt")
    ))

  have_cow <- requireNamespace("cowplot", quietly = TRUE)
  if (!have_cow && !requireNamespace("gridExtra", quietly = TRUE)) {
    stop("Please install 'cowplot' (recommended) or 'gridExtra' to build the 2×2 grid.")
  }

  get_legend <- function(p) {
    if (have_cow) cowplot::get_legend(p) else {
  ...
  if (have_cow) {
    grid_2x2 <- cowplot::plot_grid(
      p_tp_share, p_tc_share, p_tp_abs, p_tc_abs,
      labels = c("A","B","C","D"),
      label_fontfamily = ieee_family, label_size = 7, label_fontface = "bold",
      ncol = 2, align = "hv"
    )
    final <- cowplot::plot_grid(grid_2x2, shared_legend, ncol = 1, rel_heights = c(1, 0.12))
  } else {
    grid_main <- gridExtra::arrangeGrob(p_tp_share, p_tc_share, p_tp_abs, p_tc_abs, ncol = 2)
    final <- gridExtra::arrangeGrob(grid_main, shared_legend,
                                    heights = grid::unit.c(grid::unit(1, "null"), grid::unit(0.12, "null")),
                                    ncol = 1)
  }


  if (!is.null(out_base)) m3c_ieee_save(final, out_base, kind = kind, aspect = aspect)
  invisible(list(plot = final))
}
