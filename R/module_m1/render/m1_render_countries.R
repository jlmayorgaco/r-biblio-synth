...
render_m1_countries <- function(result, config = biblio_config()) {
  ...
  # F5b - Bar plot - top countries by articles
  plots$articles_bar <- ggplot2::ggplot(articles, ggplot2::aes(x = reorder(label, value), y = value)) +
    ...
    ggplot2::coord_flip() +
    ggplot2::labs(title = "Top Most Productive Countries", x = "Country", y = "Number of Articles") +
    ggplot2::theme_minimal() +
    ggplot2::theme(plot.title = ggplot2::element_text(size = 14, face = "bold", hjust = 0.5))

  # F5g - Bar plot - top countries by citations
  if ("top_countries_by_citations" %in% names(result) && nrow(result$top_countries_by_citations) > 0) {
    ...
    plots$citations_bar <- ggplot2::ggplot(citations, ggplot2::aes(x = reorder(label, value), y = value)) +
      ...
      ggplot2::coord_flip() +
      ggplot2::labs(title = "Top Countries by Total Citations", x = "Country", y = "Total Citations") +
      ggplot2::theme_minimal() +
      ggplot2::theme(plot.title = ggplot2::element_text(size = 14, face = "bold", hjust = 0.5))
  }

  # F5c - SCP/MCP stacked bar
  if ("scp_mcp_summary" %in% names(result) && nrow(result$scp_mcp_summary) > 0) {
    ...
    plots$scp_mcp_stacked <- ggplot2::ggplot(scp_mcp_long, ggplot2::aes(x = reorder(country, count), y = count, fill = type)) +
      ...
      ggplot2::coord_flip() +
      ggplot2::labs(title = "SCP vs MCP by Country", x = "Country", y = "Documents", fill = "Type") +
      ggplot2::scale_fill_manual(values = c("SCP" = palette[1], "MCP" = palette[2])) +
      ggplot2::theme_minimal() +
      ggplot2::theme(plot.title = ggplot2::element_text(size = 14, face = "bold", hjust = 0.5))
  }

  # F5d - Lorenz curve for articles
  if ("country_gini_articles" %in% names(result) && !is.na(result$country_gini_articles)) {
    ...
    plots$lorenz_articles <- ggplot2::ggplot(lorenz, ggplot2::aes(x = cumulative_x, y = cumulative_y)) +
      ...
      ggplot2::labs(title = paste0("Lorenz Curve: Country Articles (Gini = ", round(result$country_gini_articles, 3), ")"),
                    x = "Cumulative % of Articles", y = "Cumulative % of Countries") +
      ggplot2::coord_fixed(ratio = 1) +
      ggplot2::theme_minimal() +
      ggplot2::theme(plot.title = ggplot2::element_text(size = 12, face = "bold", hjust = 0.5))
  }

  # F5h - Average citations per country bar plot
  if ("avg_citations_by_country" %in% names(result) && nrow(result$avg_citations_by_country) > 0) {
    ...
    plots$avg_citations_bar <- ggplot2::ggplot(avg_cit, ggplot2::aes(x = reorder(label, value), y = value)) +
      ggplot2::geom_bar(stat = "identity", fill = palette[3], color = "black", linewidth = 0.3) +
      ggplot2::coord_flip() +
      ggplot2::labs(title = "Average Citations per Article by Country", x = "Country", y = "Average Citations") +
      ggplot2::theme_minimal() +
      ggplot2::theme(plot.title = ggplot2::element_text(size = 14, face = "bold", hjust = 0.5))
  }
